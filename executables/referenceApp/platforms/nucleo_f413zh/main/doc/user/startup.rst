...
The startup code for the NUCLEO-F413ZH platform provides the vector table, the
...
- **Exception handlers** — NMI, HardFault, MemManage, BusFault, UsageFault, SVCall,
  PendSV, SysTick.
- **IRQ handlers** — device-specific interrupt vectors (CAN1_TX, CAN1_RX0, CAN1_RX1,
  CAN1_SCE, USART3, etc.).
...
2. **FPU enable** — The Cortex-M4 single-precision FPU is enabled by setting CP10 and
...
6. **Call SystemInit** — Invokes ``SystemInit()`` (defined in ``main.cpp``) to configure
   the PLL to 96 MHz, enable the diagnostic LED, set up early USART3 output, and print
   the reset cause from ``RCC->CSR``.
...
.. note::
   ...
   file. The STM32CubeF4 default startup calls ``SystemInit()`` before
...
The linker script for STM32F413ZH defines the following memory regions:
...
.. csv-table::
   ...
   "FLASH", "0x0800_0000", "1.5 MB", "Program code, read-only data, vector table"
   "SRAM1", "0x2000_0000", "256 KB", "Main SRAM — stack, heap, .data, .bss"
   "SRAM2", "0x2004_0000", "64 KB", "Secondary SRAM — available for DMA buffers"
...
.. note::
   ...
   The STM32F413ZH has 320 KB total SRAM split across two banks. SRAM2 is accessible
   by both the core and DMA controllers, making it suitable for CAN message buffers or
   UART DMA descriptors when performance isolation is desired.
