...
initialization for the NUCLEO-G474RE platform. It aggregates the minimal set of BSP
...
1. **System timer** — ``initSystemTimer()`` configures the SysTick peripheral as the
   ...
   operations throughout the BSP and async framework. On the G474RE, SysTick is clocked
   at 170 MHz (HCLK) and is typically configured for a 1 ms tick period.

2. **UART terminal** — Retrieves the singleton UART instance designated as the
   ``TERMINAL`` channel and calls ``init()``. On the NUCLEO-G474RE, this configures
   USART2 (PA2 TX / PA3 RX) at 115200 baud via the BSP UART driver, replacing the
   raw early-boot USART2 setup performed in ``SystemInit()``. The BSP driver provides
   buffered, interrupt-driven I/O with proper framing.

.. note::

   The early USART2 configuration in ``SystemInit()`` is intentionally duplicated.
   ``SystemInit()`` provides raw TX-only output for pre-``main()`` diagnostics, while
   ``StaticBsp::init()`` initializes the full BSP UART driver with RX support, DMA (if
   configured), and integration with the ``printf``/``etl::print`` output chain.
